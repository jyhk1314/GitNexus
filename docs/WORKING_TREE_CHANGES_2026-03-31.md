...
| `gitnexus/src/core/ingestion/utils.ts` | `cppInClassCallableLabel`（类内 `Function`→`Method` 与 ingest 一致）；`getCallResolutionDebugMode`、`getCallResolutionDebugNameFilter`（`GITNEXUS_DEBUG_CALLS` / `GITNEXUS_DEBUG_CALLS_NAME`）。 |
...
| `gitnexus/test/integration/resolvers/cpp.test.ts` | 集成：`cpp-member-samefile-name-collide`、`cpp-member-field`、`cpp-qualified-call` 三类场景。 |
...
根目录 `DIFF.md` 记录与本仓库上游 **v1.4.0** 的累计差异。本节改造已在 `DIFF.md` **第五节**末尾以 **29–44** 号条目收录，避免与历史条目混写进同一条长描述。
