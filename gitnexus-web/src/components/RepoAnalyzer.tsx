...
/** Any http(s) git remote with a path (private Git / non-GitHub). */
...
export const RepoAnalyzer = ({ variant, onComplete, onCancel }: RepoAnalyzerProps) => {
  ...
  const canSubmit =
    mode === 'github'
      ? isValidGitRemoteUrl(githubUrl) && (phase === 'input' || phase === 'error')
      : localPath.trim().length > 1 && (phase === 'input' || phase === 'error');

  const handleAnalyze = async () => {
    if (mode === 'github' && !isValidGitRemoteUrl(githubUrl)) {
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <input
              ...
              disabled={isLoading}
              placeholder="https://host/org/repo"
              autoComplete="url"
              spellCheck={false}
          ...
          <label
            htmlFor={`${inputId}-branch`}
            className="mt-1 block text-xs font-medium text-text-muted"
          >
            Branch (optional — uses <code className="font-mono">repo@@branch</code> clone dir when set)
          </label>
          <input
            id={`${inputId}-branch`}
            type="text"
            ...
            autoComplete="off"
            spellCheck={false}
            className="w-full rounded-xl border border-border-default bg-void px-4 py-2.5 font-mono text-sm text-text-primary outline-none placeholder:text-text-muted focus:border-accent/40 disabled:opacity-50"
          />
        </div>
      )}
