...
const extractContent = async (
  ...
  if (node.label === 'File') {
    const MAX_FILE_CONTENT = 200_000; // ~5000 lines, avoid huge files blowing up graph storage
  ...
  const MAX_SNIPPET = 60_000; // ~1500 lines for symbol snippet (node detail / right panel)
