...
/** Extract owner `Class:X` / `Struct:X` from a class-scoped Method/Constructor nodeId. */
const tryCppOwnerClassIdFromNodeId = (nodeId: string): string | undefined => {
  if (!nodeId.startsWith('Method:') && !nodeId.startsWith('Constructor:')) return undefined;
  const stem = nodeId.slice(nodeId.indexOf(':') + 1);
...
export const enrichCppCallsTargetsFromSiblingClassScope = (graph: KnowledgeGraph): void => {
  ...
  for (const [sourceId, rels] of bySource) {
    ...
    for (const r of rels) {
      ...
      const ownerId = tryCppOwnerClassIdFromNodeId(r.targetId);
      if (!ownerId) continue;
