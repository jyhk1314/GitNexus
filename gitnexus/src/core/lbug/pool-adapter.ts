...
/**
 * Close pool entries whose LadybugDB path matches (e.g. after a local analyze rebuild).
 */
export function evictPoolsForDbPath(lbugPath: string): void {
  const want = path.resolve(lbugPath);
  for (const id of [...pool.keys()]) {
    const entry = pool.get(id);
    if (entry && path.resolve(entry.dbPath) === want) {
      closeOne(id);
    }
  }
}
